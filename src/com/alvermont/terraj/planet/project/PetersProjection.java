...
/*
 * PetersProjection.java
 *
 * Created on December 24, 2005, 2:05 PM
 *
 */
 ...
 *
 * @author martin
 * @version $Id: PetersProjection.java,v 1.9 2006/07/06 06:58:34 martin Exp $
 */
public class PetersProjection extends AbstractProjector implements Projector
{
    /** Our logging object */
    private static Log log = LogFactory.getLog(PetersProjection.class);

    /** Creates a new instance of PetersProjection */
    public PetersProjection()
    {
    }
     ...
     */
    public PetersProjection(AllPlanetParameters params)
    {
        super(params);
    }
                ...
                if ((i >= 0) && (i < width))
                {
                    for (
                        j = Math.max(
                                0,
                                (height / 2) -
                                (int) ((width * scale) / Math.PI) + k);
                            j < Math.min(
                                height,
                                (height / 2) +
                                (int) ((width * scale) / Math.PI) + k); ++j)
                        colours[i][j] = BLACK;
                }
            }
        }
